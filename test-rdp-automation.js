...
async function runTests() {
  ...
  console.log('Frontend: http://localhost:5137');
  ...
  try {
    ...
    for (let i = 1; i <= 10; i++) {
      ...
      try {
        ...
        // Wait for connection (8 seconds for WebRTC to establish)
        ...
        const videoInfo = await page.evaluate(() => {
          ...
          const statsFps = document.querySelector('.rdp-stat-value');
          ...
          return {
            ...
            fps: statsFps?.textContent?.trim() || '0'
          };
        });
        
        console.log('Video info:', videoInfo);
        
        // Determine if test passed (has video with actual dimensions)
    ...
    results.forEach(r => {
      console.log(`Test ${r.test}: ${r.passed ? '✓ PASS' : '✗ FAIL'} - Status: ${r.status}, Resolution: ${r.videoWidth}x${r.videoHeight}, FPS: ${r.fps}${r.error ? ' - Error: ' + r.error : ''}`);
    });
